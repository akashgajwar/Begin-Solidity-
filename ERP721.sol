...
contract ERC721 is IERC721{
    ...
    function getApproved(uint id)external view returns(address){
        require(owner[id] != address(0), "token does not exists");
        return _approvals[id];
    }

    function _isApprovedOwner(address owner, address spender, uint id) internal view returns (bool) {
        require(spendr == owner || isApprovedForAll[owner][spender] || spender == _approvals[id]);
    ...
    function transferFrom(address from, address to, uint id) public{
        ...
        require(isApprovedForAll(from, msg.sender, id), "Not authorised");
        _balanceOf[from]--;
        _balanceOf[to]++;
        _ownerOf[id] = to;
    ...
    //     IERC721Receiver.onERC721Received.selector, "unsafe recipent");
    // }
...
contract MyNFT is ERC721{
    ...
    function Transfer(address from, address to, uint id) external{
        transferFrom(from, to, id);
    }
